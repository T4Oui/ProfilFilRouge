...
void recherche_comparaison_texte(char * pathFileRecherchee, int nbFichierTxt){
    ...
    while(fscanf(listeRechTriee,"%s",chemin)==1){
        ...
        */
        int ret = snprintf(ecrireResult,sizeof(ecrireResult),"echo '%s' >> /home/pfr/pfr_code/data/resultat_recherche.txt",chemin);
        if(ret<0){
            abort();
        } system(ecrireResult);
    } 
            ...
            while (fscanf(rech,"%d",&id_fich)==1){
                ...
                while (fscanf(base,"%d",&id_courant) == 1){
                    ...
                    if(id_courant==id_fich){
                        //printf("%s %d\n",chemin,nb);
                        int ret = snprintf(ecrireResult,sizeof(ecrireResult),"echo '%s %d' >> /home/pfr/pfr_code/data/resultat_recherche.txt",chemin,nb);
                        if(ret<0){
                            abort();
                        } system(ecrireResult);
                    }
